<h1 align="center">👋 Hey there! I'm Alex</h1>
<h3 align="center">MVA Student @ ENS Paris-Saclay</h3>

<p align="center">
  <b>Mathematics</b> &nbsp;|&nbsp; <b>Deep Learning</b> &nbsp;|&nbsp; <b>Data Science</b>
</p>

<p align="center">
  I'm passionate about machine learning, deep learning, and software engineering. <br>
  Currently exploring new technologies and working on personal projects in my free time.
</p>

---

<div align="center">
  
### ✨ &nbsp;Latest Projects
| Project | Description | Stack |
| :--- | :--- | :--- |
| 📉 **[Stochastic FISTA](https://github.com/TopAgrume/Stochastic_FISTA)** | Stochastic accelerated proximal optimization algorithm implementation. | `Python` |
| 🧩 **[DVLWDL](https://github.com/mva-ii/Discriminative-Dictionary-Learning-for-Time-Series-Classification)** | Discriminative dictionary learning for time series classification. | `Python` |
| 🧠 **[Cuda Neural Network](https://github.com/FlorianSegard/CudaNeuralNetwork)** | Custom neural network implementation with CPU/GPU support (FWD & BWD). | `C++` `CUDA` |
| 🎞️ **[GPGPU](https://github.com/FlorianSegard/GPGPU_Project)** | Video motion estimation project leveraging GPGPU techniques. | `C++` `CUDA` |
| 🕹️ **[Atari Rainbow DQN](https://github.com/TopAgrume/Atari-Rainbow-DQN)** | A reinforcement learning project designed to play Atari Breakout. | `PyTorch` |
| 🎯 **[MLVOT](https://github.com/TopAgrume/mlvot)** | Multi-object tracking using Kalman Filters. | `PyTorch` |
| 🕸️ **[I-JEPA](https://github.com/DjDonPablo/I-JEPA)** | Implementation of Meta's Joint Embedding Predictive Architecture. | `PyTorch` |

### 🏆 &nbsp;Hackathon Achievements

| Event | Project & Outcome | Stack |
| :--- | :--- | :--- |
| 🎗️ **PINKCC Lab** | **[Finalist]** DL for 3D CT scan segmentation of ovarian cancer. | `PyTorch` |
| 🎖️ **Armed Forces** | **[Finalist]** ResNet18 fine-tuning for car classification. | `PyTorch` |
| 🤖 **Mistral AI** | **[ASCII Art]** Fine-tuned Mistral-7b model for ASCII art generation (Discord bot). | `Python` |
| ⚕️ **Mistral AI** | **[Hygie.ia]** AI-powered platform for automating healthcare assessments. | `Next.js` |

### 🎯 &nbsp;Featured & Academic

| Project | Description | Stack |
| :--- | :--- | :--- |
| 🚁 **[SkyGuards](https://github.com/TopAgrume/SkyGuards)** | Real-time crowd density monitoring system using drones. | `Scala` |
| 📊 **[Big Data](https://github.com/xCosmicOtter/bigdata)** | Large-scale stock data processing and visualization. | `Pandas` `Dash` |
| 📝 **[NLP Poetics](https://github.com/TopAgrume/NLP_Project)** | NLP project for poem analysis and creation. | `Jupyter` |
| 🤖 **[ML Projects](https://github.com/TopAgrume/ml_projetcs)** | Ray, Optuna, parallelism, ALS recommender systems, KMeans, PCA. | `Jupyter` |
| 🎨 **[DECOLOR](https://github.com/TopAgrume/decolor)** | Image editing and drawing software. | `C` |
| 🧩 **[OCR Sudoku](https://github.com/TopAgrume/OCR_LMP)** | Optical Character Recognition for solving Sudoku puzzles. | `C` |
| 🎮 **[ERASED](https://github.com/Akaagi/Erased)** | Metroidvania-style game developed with Unity. | `C#` |

</div>
<br>

### 💡 &nbsp;Personal Tools

- 🛡️ &nbsp;**[Safeguard.io](https://github.com/TopAgrume/safeguard.io)** - Safety solution for people in difficulty / isolated `Python` `SQL`
- 📝 &nbsp;**[Auto Changelogs](https://github.com/TopAgrume/auto_changelog)** - Streamlined changelog management across branches `Bash`
- 🌡️ &nbsp;**[Raspberry Pi Fan Control](https://github.com/TopAgrume/raspberrypi-fan-control)** - PWM-based fan control for Raspberry Pi 4B `Python`

---

### 🛠️ &nbsp;Tech Stack

<div align="center">

**Languages**<br>
<img alt="Python" src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
<img alt="C++" src="https://img.shields.io/badge/C%2B%2B-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" />
<img alt="C" src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" />
<img alt="Java" src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" />
<img alt="Scala" src="https://img.shields.io/badge/Scala-DC322F?style=for-the-badge&logo=scala&logoColor=white" />
<img alt="Bash" src="https://img.shields.io/badge/GNU%20Bash-4EAA25?style=for-the-badge&logo=GNU%20Bash&logoColor=white" />

**Machine Learning & Data Science**<br>
<img alt="PyTorch" src="https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=PyTorch&logoColor=white" />
<img alt="NumPy" src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white" />
<img alt="Pandas" src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" />
<img alt="Scikit-Learn" src="https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white" />
<img alt="Matplotlib" src="https://img.shields.io/badge/Matplotlib-11557c?style=for-the-badge&logo=python&logoColor=white" />
<img alt="CUDA" src="https://img.shields.io/badge/CUDA-76B900?style=for-the-badge&logo=nvidia&logoColor=white" />

**Big Data & Backend**<br>
<img alt="Apache Spark" src="https://img.shields.io/badge/Apache%20Spark-FFFFFF?style=for-the-badge&logo=apachespark&logoColor=#E35A16" />
<img alt="Apache Kafka" src="https://img.shields.io/badge/Apache%20Kafka-231F20?style=for-the-badge&logo=apache-kafka&logoColor=white" />
<img alt="PostgreSQL" src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white" />
<img alt="Microsoft Azure" src="https://img.shields.io/badge/Microsoft%20Azure-0078D4?style=for-the-badge&logo=microsoft%20azure&logoColor=white" />
<img alt="Docker" src="https://img.shields.io/badge/Docker-2CA5E0?style=for-the-badge&logo=docker&logoColor=white" />
<img alt="Dash" src="https://img.shields.io/badge/Dash-008DE4?style=for-the-badge&logo=dash&logoColor=white" />
<img alt="Triton Inference Server" src="https://img.shields.io/badge/Triton_Inference_Server-76B900?style=for-the-badge&logo=nvidia&logoColor=white" />

</div>

---
</tr>
</table>
